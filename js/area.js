...
function selchange(v){
	var str="0";
	for(i=0;i<v;i++){
		str+=("_"+(document.getElementById(sel[i]).selectedIndex-1));
	};
	var ss=document.getElementById(sel[v]);
	with(ss){
		length = 0;
		options[0]=new Option(opts[v],opts[v]);
		if(v && document.getElementById(sel[v-1]).selectedIndex>0 || !v){
			if(dsy.Exists(str)){
				ar = dsy.Items[str];
...
dsy.add("0",["北京市","天津市","上海市","重庆市","河北省","山西省","内蒙古","辽宁省","吉林省","黑龙江省","江苏省","浙江省","安徽省","福建省","江西省","山东省","河南省","湖北省","湖南省","广东省","广西","海南省","四川省","贵州省","云南省","西藏","陕西省","甘肃省","青海省","宁夏","新疆","香港","澳门","台湾省"]);
var sel=["selProvince","selCity","selCounty"];//三个select的name
var opts = ["省份","地级市","市、县级市"];//初始值
function initarea(){  //初始化函数
	for(i=0;i<sel.length-1;i++){
	  document.getElementById(sel[i]).onchange=new Function("selchange("+(i+1)+")");
	}
	selchange(0);
}
//添加弹窗
...
//修改弹窗
var a=["province","city","county"];//三个select的name
var opt1 = ["省份","地级市","市、县级市"];//初始值
function init_area(){  //初始化函数
	for(i=0;i<a.length-1;i++){
	  document.getElementById(a[i]).onchange=new Function("echange("+(i+1)+")");
	}
	echange(0);
}